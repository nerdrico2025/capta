...
const SOURCE = 'BNDES';
const PAGE_URL =
  'https://www.bndes.gov.br/wps/portal/site/home/onde-atuamos/inovacao-e-desenvolvimento/chamadas-publicas/chamadas-publicas-abertas';
const UA = 'Mozilla/5.0 (compatible; CaptaBot/1.0; +https://capta.org.br)';

const OSC_FILTER =
  /social|cultura|educa[çc][ãa]o|sa[úu]de|meio ambiente|comunidade|terceiro setor|OSC|organiza[çc][ãa]o|vulnerab|assist[êe]ncia|humanit/i;
...
export class BndesCollector {
  ...
  async run({ prisma }: { prisma: PrismaClient }): Promise<CollectorResult> {
    ...
    try {
      ...
      if (html) {
        let blocks = extractBlocks(html);

        // HTML estático do WebSphere Portal não renderiza o conteúdo — forçar Puppeteer
        if (blocks.length === 0) {
          console.info(`[${SOURCE}] Nenhum bloco no HTML estático — usando Puppeteer`);
          try {
            const { fetchWithPuppeteer } = await import('../utils/puppeteer.js');
            const puppeteerHtml = await fetchWithPuppeteer(PAGE_URL);
            blocks = extractBlocks(puppeteerHtml);
          } catch (puppeteerErr) {
            console.warn(`[${SOURCE}] Puppeteer falhou:`, puppeteerErr);
          }
        }
